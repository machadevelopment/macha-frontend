...
Stack: Next.js (App Router) · React 18 · TypeScript strict · Tailwind CSS · shadcn/ui (Radix) · Tremor Raw (charts/KPIs, on Recharts) · Lucide React · `next-themes` · i18n ES/EN · SF Pro Display vía stack del sistema, con Inter + JetBrains Mono por `next/font`.
...
- **Tipografía: SF Pro Display, con Inter de respaldo.** SF Pro **no se empaqueta** — es de Apple, su licencia la restringe a plataformas Apple y no está en Google Fonts, así que auto-hospedarla sería una violación. Se pide por el stack del sistema (`--font-ui-stack` en `globals.css`): resuelve a SF en macOS/iOS y cae a Inter en Windows/Android, que por eso se sigue cargando con `next/font`. **Consecuencia aceptada: el producto no se ve igual en Mac que en Windows.**
